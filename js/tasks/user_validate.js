...
validator = {
	...
	config: {
		...
		heading: "Script health report for the <b>user namespace</b> of {{subst:SITENAME}}",
	...
	launch: function() {
		console.log('User script validate here. Validating user script pages.');
	...
	$processTitleValidationResults: function( pg ) {
		...
		validator.$updatePageStatus( pg ).done(function() {
			...
			$.each(notificationsByUser, function(user, msgs) {
				...
				validator.client.api.call({
					action: 'query',
					prop: 'categories',
					clcategories: 'Category:Users opting out validation spam',
					titles: 'User:' + user
				}, function( r ) {
	...
	$fetchworkUntil: function() {
		...
		if (!validator.workUntil) {
			validator.bot.fetchSetting( 'user_last_rc_timestamp', function(err, results) {
				if (err) {
					$def.reject(err);
				} else {
